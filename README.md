...
A service that on a daily schedule queries the MEMO database to pick MEMO tasks that are due to be run. 
...
On each run an existing older version of the output file is overwritten.
...
Python scripts are inspected by the Task runner before being executed. Though they could not do much harm, as they are run inside a Docker container, certain commands are forbidden.
...
Python libraries are *not* inspected, so when any of these instructions occur in an installed library, that could compromise security. Then again, Python libraries can only be installed by a system administrator.

When Task runner encounters any illegal instruction the script will not run and an error is thrown and logged:
...
* `path.temp.pythonscripts`   
   ...
   to not remove the generated Python files after the task is run (setting `path.temp.pythonscripts.purge`)    
...
* `path.temp.pythonscripts.purge`   
   Remove temporary Python files tree for tasks that have finished running (set to `true`)  
* `path.exports`   
...
* `taskrunner.cronschedule`   
   MEMO Taskrunner runs continously as a service, but we periodically need to select and run the defined tasks. To set a scheme
   for this purpose Cron-style syntax is used (suggested value `0 0 2 * * *`, 2.00 AM every day).   
   (`* * * * * *` = second, minute, hour, day of month, month, and day of week)
* `taskRunner.busy.starttime`   
* `taskRunner.busy.hours`   
   This denotes a period during which no client calls (run or dry run requests through http) will be processed.
   The period should start at the same time as `cronschedule` and extend for enough hours to reasonably allow the task runner 
   to finish all its selected tasks.    
   (suggested values `starttime=02:00` and `hours=4`)

### Export directory structure

The export directory at `path.exports` has a sub-directory for every client containing the files that are generated by the task runner.
Also a directory `tmp` is available containing the results of dry runs.
...
These directories at maximum hold just as many files as there are tasks, because older versions of the files will be overwritten each time a task runs.	  
...
Output will always be suppressed (`null`), because we do not want to flood our browser screen with actual file contents.	
